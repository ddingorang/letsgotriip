...
3. ✅ 2026-06-18 완료 — 챌린지/뱃지 `gamification`: 별도 엔티티/이벤트 없이 기존 계획·장소 수에서 **읽기 시점 파생**. `PlanRepository` 집계 3메서드(countByUserId/countPlacesByUserId/countCompletedByUserId) + `GamificationService`(뱃지 6종·월 챌린지 10곳 정의) + `GET /api/gamification/summary`. FE `gamificationApi`+`stores/gamification.js`, MyPage 통계·챌린지카드·뱃지탭 / ChallengeDetailView / BadgesView 뱃지그리드 실연동. **검증**: kim 2계획·5장소→첫여행·탐험가 획득, 플래너 2/3. (BadgesView 퀘스트/레벨/XP는 별도 리치모델이라 보류)
4. ✅ 2026-06-18 완료 — 동선리포트(거리 소스=**Haversine, 외부API 없음**): `plan/util/RouteCalculator`(haversine+nearest-neighbor) + `RouteReportResponseDto` + `GET /api/plans/{id}/route-report`. 좌표는 `AttractionSummaryDto.latitude/longitude`(이미 plan 응답에 포함). "대체 동선 적용"=추천순서로 기존 `PUT .../places`(replacePlaces) 재호출. FE PlanReportView 실데이터(가짜 추정 제거). **검증**: 4장소 85km, 구간거리 실측, nearest-neighbor가 더 짧은 순서 추천(reorderSuggested).
