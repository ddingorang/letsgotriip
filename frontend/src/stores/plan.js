...
export const usePlanStore = defineStore('plan', () => {
  ...
  async function loadPlans() {
    ...
    error.value = null;
    try {
      // 백엔드는 Spring Page 객체({content:[...]})를 반환하고 page는 0-base
  ...
  async function createPlan(payload) {
    ...
    try {
      ...
      // Re-fetch list and set current to the newly created plan
      await loadPlans();
  ...
  async function updatePlan(payload) {
    // payload must include id and expectedVersion
    loading.value = true;
    error.value = null;
    try {
      ...
      if (code === 'PLAN4092') {
        // Version conflict — re-sync and inform user
  ...
  async function replacePlaces(planId, dayNo, places) {
    // places: array of place objects from current store (seq will be recalculated)
    loading.value = true;
    error.value = null;
    try {
      const body = {
